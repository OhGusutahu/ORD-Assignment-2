...
/**
    Função que insere uma chave, seu offset e seu filho direito em uma página.
    
    Entradas:
    => int chv_pro = chave a ser inserida
    => int offset_pro = offset da chave a ser inserida
    => int rrn_pro = filho direito da chave a ser inserida
    => int chaves[] = campo chaves da página
    => int offsets[] = campo offsets (das chaves) da página
...
/**
    ...
    => int offset_i = offset da chave a ser inseirda
    ...
    => int *offset_pro = offset da chave promovida
...
/**
    ...
    => int offset = offset da chave no arq_dados
...
/**
    ...
    => int *pos = arg de retorno
        - posição em que a chave está (se estiver na página)
                ou deveria estar (se não estiver na página)
...
/**
    ...
    => int chave = chave a ser buscada
    => int *rrn_encontrado = arg de retorno
            - caso a chave seja encontrada, conterá o RRN da página em que ela está
    => int *pos_encontrada = arg de retorno
            - caso a chave seja encontrada, conterá a posição da chave na página de rrn_encontrada
